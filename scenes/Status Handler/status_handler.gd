...
const STATUA_APPLY_INTERVAL := 0.25
const STATUS_UI = preload("res://scenes/Status Handler/status_ui.tscn")
...
func apply_statuses_by_type(type: Status.Type) -> void:
	...
	for status: Status in status_queue:
		...
		tween.tween_interval(STATUA_APPLY_INTERVAL)
...
func add_status(status: Status) -> void:
	...
	if not has_status(status.id):
		...
		new_status_ui.status.status_applied.connect(on_status_applied)
		new_status_ui.status.initialize_status(status_owner)
		return
	
	# return if unique & we have it
	if not status.can_expire and not stackable:
		return
	
	# expanding the duration stackable effect
