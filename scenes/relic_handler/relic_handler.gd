...
@onready var relics_control: RelicsControl = $"Relics Control"
@onready var relics: HBoxContainer = %Relics


func _ready() -> void:
	relics.child_exiting_tree.connect(on_relics_child_exiting)
...
func activate_relics_by_type(type: Relic.Type) -> void:
	...
	var relic_queue: Array[RelicUI] = get_all_relic_ui_nodes().filter(
...
func get_all_relics() -> Array[Relic]:
	var relic_ui_nodes := get_all_relic_ui_nodes()
...
func get_all_relic_ui_nodes() -> Array[RelicUI]:
...
func on_relics_child_exiting(relic_ui: RelicUI) -> void:
	if not relic_ui:
		return
