...
const MOUSE_Y_SNAPBACK_THREHOLD := 138
...
func enter() -> void:
	...
	Events.card_aim_starts.emit(card_ui)
	


func exit() -> void:
	Events.card_aim_ends.emit(card_ui)



func on_input(event: InputEvent) -> void:
	var mouse_motion := event is InputEventMouseMotion
	var mouse_at_bottom := card_ui.get_global_mouse_position().y > MOUSE_Y_SNAPBACK_THREHOLD
